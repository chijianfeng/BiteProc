...
BOOL CMainFrame::PreCreateWindow(CREATESTRUCT& cs)
{
	...
	m_strTitle = m_UserCfg.GetCurUser() + L"用户";
...
void CMainFrame::OnDevicesettingSeldevice()
{
	...
	if (dlg.DoModal() == IDOK)
	{
		...
		if (pDoc == NULL)
			return ;
		m_wndOutput << "修改设备" << m_EndOut;
		pDoc->DeleteDevice();
		LoadDevice();
	}
	else
		m_wndOutput << "未修改设备" << m_EndOut;
}
...
void CMainFrame::OnSettingAlogrithm()
{
	...
	if (dlg.DoModal() == IDOK)
	{
		...
		if (pDoc == NULL)
			return;
		//获取当前模块或者应用
		CBaseTrainWnd* pTrainWnd = pDoc->GetBaseTrainWnd();				//获取当前的训练模块
		if (pTrainWnd)
		{
			CString path;
			...
			pTrainWnd->SetParm(config);										//设置参数
			pTrainWnd->SendMessage(WM_PAINT);
		}
		else
			m_wndOutput << "模块不存在，无法设置参数" << m_EndOut;
	}
	else
		m_wndOutput << "未设置训练模块" << m_EndOut;
}
...
LRESULT CMainFrame::OnInnerMessage(WPARAM wparam, LPARAM lparam)
{
	...
	case INNERMAINFRAME_UPDATA:
		...
		if (pDoc != NULL)
		{
			...
			pDoc->UpdateAllViews(NULL, 0L, &m_Rng);				//更新
		}
		break;
	case  INNERMAINFRAME_CMDCODE:
		m_wndOutput << "识别命令为：" << lparam << m_EndOut;
		break;
	default:
		break;
	}
...
bool CMainFrame::LoadDevice()
{
	...
	CString dir;
	m_UserCfg.GetUserPath(m_UserCfg.GetCurUser(), dir);
	CString path = dir + _T("\\"+DEVICEFILE);				//设备参数配置文件
	...
	{
		m_wndOutput << "Load "<< name<<" Device Failed!" << m_EndOut;		//加载设备失败
...
BOOL CMainFrame::OnCommand(WPARAM wParam, LPARAM lParam)
{
	...
	{
		return (BOOL)pWnd->SendMessage(wParam, 0, 0);				//发生消息
	}

	if (pTrainLoader->CheckMenuID((UINT)wParam)	)					//训练模块消息
	{
		if (pTrainWnd)
